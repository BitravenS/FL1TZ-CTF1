...
def dialogue3():
    fprint(f"{COLOR_BLUE} *Hallway lights up* {COLOR_RESET}")
    fprint(
        f"> FINALLY, I mean {COLOR_YELLOW}WHO EVEN IMPLEMENTS ELLIPTIC CURVE ENCRYPTION IN THEIR DAMN GENERATOR?!{COLOR_RESET}"
    )
    fprint(f"> Anyway, this should be it, {COLOR_YELLOW}hopefully{COLOR_RESET}")
    fprint(f"""{COLOR_BLUE}>>> Welcome to the TerraGroup HQ{COLOR_RESET}""")
    fprint(
        f"""{COLOR_BLUE}>>> The LABS™ are the most sensetive parts of our facility{COLOR_RESET}"""
    )
    fprint(
        f"""{COLOR_BLUE}>>> As such, only the top few scientist in the world are authorized to access this area{COLOR_RESET}"""
    )
    sleep(1)
    fprint(f"> Let me guess... I have 3 attemptes?")
    fprint(f"{COLOR_RED}>>> YOU HAVE 1 ATTEMPT{COLOR_RESET}")
    fprint(f"> WAIT WHAT?????????????????????")
    sleep(1)
    fprint(f"{COLOR_RED}>>> ...TO VALIDATE ACCESS{COLOR_RESET}")
    fprint(f"> Oh, okay. That's better, I guess. So I can do other stuff, right?")
    fprint(f"> Let's see what we have here")
