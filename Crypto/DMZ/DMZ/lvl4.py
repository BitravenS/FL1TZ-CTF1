...
def dialogue4():
    announcer("ATTENTION! ATTENTION!, THIS IS NOT A DRILL", 0.005, lower=False)
    announcer(
        "ALL PERSONNEL ARE TO EVACUATE THE LABS IMMEDIATELY",
        0.005,
        upper=False,
        lower=False,
    )
    announcer(
        "A Breach of the facility has been detected", 0.005, upper=False, lower=False
    )
    announcer(f"Commencing full lockdown in {COLOR_RED}60 SECONDS", 0.001, upper=False)
    fprint(
        f"> I don't have much time left, I need to get the cure and leave {COLOR_YELLOW}ASAP{COLOR_RESET}"
    )

    fprint(f"> It should be behind that vault door")
    fprint(f"> Man... This lab is HUUUGE")
    fprint(
        f"> I can see a {COLOR_YELLOW}note{COLOR_RESET} on the desk, a {COLOR_YELLOW}Fingerprint Scanner{COLOR_RESET} to open the vault, I think, and a {COLOR_YELLOW}whiteboard{COLOR_RESET} with some scribbles. Oh, and a {COLOR_YELLOW}test bench {COLOR_RESET}for, something."
    )
    announcer(f"50 SECONDS REMAINING")
    fprint(f"> I need to hurry")
...
def level4():
    ...
    dialogue4()
    fprint(
        f"\n{COLOR_YELLOW}! Connect on port 1012 to disable animations, colors and dialogue (PS: They might contain hints).{COLOR_RESET}\n"
    )
    fprint(
        f"""{COLOR_YELLOW}
